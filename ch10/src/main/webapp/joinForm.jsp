...
<!-- 이 프로그램에서 jquery 사용 할거야 -->
<script type="text/javascript" src="js/jquery.js"></script>
<script type="text/javascript">
	function chk() {
		if (frm.password.value != frm.password2.value) {
			alert("암호화 암호확인이 다릅니다");
			frm.password.focus();
			frm.password.value = "";
			frm.password2.value = "";
			return false;
	...
	function dupChk() {
		...
		/* $ : jQuery 사용한다는 뜻/ post : 메서드가 post 방식 */
		$.post('confirm2.jsp','id='+frm.id.value, function(data) {
			/* #은 id, .은 클래스 */
			$('#err').html(data);
		}); 
		/* window.open : 팝업창으로 띄워줘 => 옛날 방식*/
...
<table><caption>회원 가입</caption>
	...
	<!-- pattern="010-\d{3,4}-\d{4}" : 전화번호는 010-숫자 3 또는 4자리 - 숫자 4자리
		placeholder="010-1111-1111" : 처음에 보여주고 데이터가 입력되면 사라짐
